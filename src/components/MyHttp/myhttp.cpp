...
void MyHttp::post_async(const QString &url,
                        ...
                        std::function<void(QString)> onError)
{
    ...
    connect(reply, &QNetworkReply::finished, this, [=]() {
        ...
        if (err.error != QJsonParseError::NoError || !doc.isObject()) {
            if (onError)
                onError("JSON Parse Error: " + err.errorString());
            return;
        }
...
void MyHttp::postImage(const QString &url,
...
void MyHttp::getImage(const QString &url,
...
void MyHttp::get_async(const QString &url,
                       std::function<void(QJsonObject)> onSuccess,
                       std::function<void(QString)> onError)
{
    QUrl qurl(url);
