...
class MyChartView : public QChartView {
 ...
 public:
  ...
  void recordInitialAxisRange() {
    ...
    m_initialRange =
        QRectF(m_axisX->min(), m_axisY->min(), m_axisX->max() - m_axisX->min(), m_axisY->max() - m_axisY->min());
  }

  void backInitialRange() {
    if (!m_initialRange.isValid() || !m_axisX || !m_axisY) return;
 ...
 protected:
  void mouseMoveEvent(QMouseEvent *event) override {
    ...
    QString coordText = QString("X: %1\nY: %2").arg(closestPoint.x(), 0, 'f', 3).arg(closestPoint.y(), 0, 'f', 3);
