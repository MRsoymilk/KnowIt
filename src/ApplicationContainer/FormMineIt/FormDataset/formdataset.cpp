...
void FormDataset::on_tableView_clicked(const QModelIndex &index)
{
    ...
    QString url = QString("%1%2").arg(MY_GLOBAL->get<QString>(URL_SERVER),
                                      MY_GLOBAL->get<QString>(PATH_DATASET_GET));
    ...
    QString url_get_img_spectral = QString("%1%2/%3/%4")
                                       .arg(MY_GLOBAL->get<QString>(URL_SERVER),
                                            ...
                                            objInfo["ID"].toString(),
                                            objBasicInfo["Structure Picture"].toString());
    MY_HTTP->getImage(
        url_get_img_spectral,
        [=](QPixmap pix) { emit itSpectral(pix); },
        [=](QString err) {
