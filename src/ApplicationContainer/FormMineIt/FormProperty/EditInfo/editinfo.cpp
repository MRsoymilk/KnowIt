...
QJsonObject EditInfo::getData()
{
    ...
    obj.insert(SPECTRAL_ACQUISITION_CONDITIONS,
               m_SpectralAcquisitionConditions->getSpectralAcquisitionConditions());
...
void EditInfo::onStructure(const QPixmap &pix)
{
    emit sendStructure(pix);
}

void EditInfo::onSpectral(const QPixmap &pix)
{
