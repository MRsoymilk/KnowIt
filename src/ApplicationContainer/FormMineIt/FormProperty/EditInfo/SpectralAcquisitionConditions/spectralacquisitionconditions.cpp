...
SpectralAcquisitionConditions::SpectralAcquisitionConditions(QWidget *parent)
    : QWidget(parent), ui(new Ui::SpectralAcquisitionConditions) {
  ui->setupUi(this);
}

SpectralAcquisitionConditions::~SpectralAcquisitionConditions() { delete ui; }
...
void SpectralAcquisitionConditions::setSpectralAcquisitionConditions(const QJsonObject &data) {
  ...
  ui->lineEditInstrumentModel->setText(objSpectralAcquisitionConditions.value(INSTRUMENT_MODEL).toString());
  ui->lineEditLaserWavelength->setText(objSpectralAcquisitionConditions.value(LASER_WAVELENGTH).toString());
  ui->lineEditLaserPower->setText(objSpectralAcquisitionConditions.value(LASER_POWER).toString());
  ui->lineEditCalculusTimes->setText(objSpectralAcquisitionConditions.value(CALCULUS_TIMES).toString());
  ui->lineEditMeasuringEnvironment->setText(objSpectralAcquisitionConditions.value(MEASURING_ENVIRONMENT).toString());
  ui->lineEditMeasuringTemperature->setText(objSpectralAcquisitionConditions.value(MEASURING_TEMPERATURE).toString());
}
...
QJsonObject SpectralAcquisitionConditions::getSpectralAcquisitionConditions() {
  ...
  objSpectralAcquisitionConditions.insert(MEASURING_ENVIRONMENT, ui->lineEditMeasuringEnvironment->text());
  objSpectralAcquisitionConditions.insert(MEASURING_TEMPERATURE, ui->lineEditMeasuringTemperature->text());
