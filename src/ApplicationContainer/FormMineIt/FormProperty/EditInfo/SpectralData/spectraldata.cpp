...
void SpectralData::setID(const QString &id)
{
    m_id = id;
}

void SpectralData::setUUID(const QString &uuid)
{
    m_uuid = uuid;
}
...
bool SpectralData::drawPoint(const QPointF &scenePos,
                             const int &pixelX,
                             const int &pixelY,
...
bool SpectralData::eventFilter(QObject *watched, QEvent *event)
{
    ...
    if (watched == ui->graphicsViewSpectral->viewport()
